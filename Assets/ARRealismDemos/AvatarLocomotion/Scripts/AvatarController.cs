...
public class AvatarController : MonoBehaviour
{
    ...
    public void ToggleMove(Transform target)
    {
        isMove = true;
        this.target = target;
    }
    ...
    public void MoveToward(Transform target)
    {
        ...
        Vector3 newLookAtPosition = new Vector3(target.transform.position.x,
          myTransform.position.y, target.transform.position.z);
        ...
        if (currentDistance < _distanceToPickUp ||
           target.transform.position.y < -_maxChaseDistanceInMeter)
        {
            isMove = false;
            currentDistance = 0;
            LookAtCamera();
